...
- [ ] **17.4** Reorganize the repository structure to make the root simpler for newcomers by clarifying what to read/run first, and move non-essential root scripts/files (for example `dispatch-dev.ps1`, `dispatch-dev.sh`, and similar files that can be nested without breaking behavior) into logical subfolders.
